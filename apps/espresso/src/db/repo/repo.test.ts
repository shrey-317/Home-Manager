...
import { collectPending, markPushed, pendingCount, syncOnce } from '../sync/outbox.ts';
...
describe('write stamping', () => {
  it('stamps the envelope and queues an outbox entry on create', async () => {
    ...
    expect(pending[0]!.upserts).toHaveLength(1);
  });
...
describe('tombstones', () => {
  it('hides deleted rows from reads but keeps them for sync', async () => {
    ...
    const pending = await collectPending(db);
    expect(pending[0]!.deletes).toEqual([bean.id]);
    expect(pending[0]!.upserts).toHaveLength(0);
  });
...
describe('outbox coalescing', () => {
  it('sends one upsert for a row edited several times', async () => {
    const bean = await beansRepo.create({ roaster: 'JVG', name: 'Sidama', state: 'active' }, db);
    ...
    expect((pending[0]!.upserts[0] as { notes: string }).notes).toBe('second');
  });
...
describe('sync seam', () => {
  /** No adapter ships in v1; this fake proves the seam works end to end. */
  function fakeAdapter(incoming: PushBatch[] = []): SyncAdapter & { pushed: PushBatch[][] } {
    ...
    return {
      ...
      async push(batches) {
        pushed.push(batches);
        const seqs = (await db.outbox.toArray()).map((e) => e.seq!);
        return { acceptedSeqs: seqs };
      },
      async pull() {
        return { batches: incoming, serverTime: Date.now() };
      },
    };
  }
  ...
  it('accepts a newer remote row and rejects a staler one', async () => {
    const bean = await beansRepo.create({ roaster: 'JVG', name: 'Sidama', state: 'active' }, db);
    await markPushed((await db.outbox.toArray()).map((e) => e.seq!), db);

    const stale = { ...bean, roaster: 'STALE', updatedAt: bean.updatedAt - 1000, dirty: 0 as const };
    await syncOnce(fakeAdapter([{ table: 'beans', upserts: [stale], deletes: [] }]), 0, db);
    ...
    await syncOnce(fakeAdapter([{ table: 'beans', upserts: [fresh], deletes: [] }]), 0, db);
  ...
  it('lets a local tombstone win over a remote edit', async () => {
    ...
    await syncOnce(fakeAdapter([{ table: 'beans', upserts: [revived], deletes: [] }]), 0, db);
